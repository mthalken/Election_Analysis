...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_vote:
        ...
        total_county_votes = county_vote[county_name]

        # 6c: Calculate the percentage of votes for the county.
        county_vote_percentage = round(float(total_county_votes) / float(total_votes) * 100, 2)

    # 6d: Print the county results to the terminal.
        print(f"{county_name}: {county_vote_percentage:.1f}% ({total_county_votes:,})\n")

    # 6e: Save the county votes to a text file.
        txt_file.write(f"{county_name}: {county_vote_percentage:.1f}% ({total_county_votes:,})\n")
